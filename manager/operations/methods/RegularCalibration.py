...
class RegularCalibration(method.AnalysisMethod):
    can_be_applied = True
    _steps = [
        {
            'class': SelectAnalyte,
    ...
    description = """
This is standard calibration method, where the height of the signal is
calculated as a difference between max and min signal in the given range.
This procedure creates a model which can be applied to other curve(s) to
obtain the final result.
""".replace('\n', ' ')

    @classmethod
    def __str__(cls):
        return "Regular Calibration"
    ...
    def finalize(self, user):
        ...
        __, sslope, sintercept = calc_sx0(p['slope'], p['intercept'], data[0], data[1])
        if p['slope'] != 0:
