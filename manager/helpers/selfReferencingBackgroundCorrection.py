...
def selfReferencingBackgroundCorrection(yvectors, concs, sens, peak_ranges):
    """
    Implements Self Referencing Backgound Correction:
    yvectors -- matrix: each column should be one measurement.
    concs -- list: containing the concentrations of analytes in
        measurements in the same order as yvectors
    sens -- list: contatning sensitivity of each measurement in
        the same order as yvectos
    peak_ranges -- list: min and max index of peak of interest
    """
    ...
    for width_bkg in width_vector:
        ...
        for pos_bkg in span_vector:
            ...
            for push_bkg in ecc_vector:
                if any([
                    ((peak_max - push_bkg - pos_bkg - width_bkg) < 1),
                    ((peak_max + push_bkg + pos_bkg + width_bkg) > len(sig))
                ]):
                    # OUT OF BOUNDS
                    continue
                push_bkg_ctr += 1
