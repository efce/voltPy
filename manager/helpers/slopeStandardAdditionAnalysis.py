...
# ==============================================================
    assert all([
        len(DATACELL['SENS']) == len(DATACELL['Y']),
        len(DATACELL['CONC']) == len(DATACELL['Y'])
    ])
    ...
    for r_l_avg, r in result.items():
        ...
        for s, d in r['Slopes'].items():
            for ss, dd in r['Slopes'].items():
                ...
                # ==========================================================
                if any([
                    slopeProportion > (1+slopeDiffRequired),
                    slopeProportion < (1-slopeDiffRequired)
                ]):
                    ...
                    #                    ])
                    x = (
                        (r['Fit'][s]['intercept']-r['Fit'][ss]['intercept'])
...
def getSlopeInInflection(signal, peak, forceFitRange, fitRange):
    ...
    if (None not in fitRange) and forceFitRange:
        ...
        for fitPos in np.arange(peak, fitSize, -1):
            ...
            if None is prevNormalFit.get('slope', None):
                prevNormalFit = fitL
        ...
        if None is finalNormalFit.get('slope', None): 
            print('Fit failed')
        else:
